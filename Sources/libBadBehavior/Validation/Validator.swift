        ...
        await withThrowingTaskGroup(of: Void.self) { group in
            for flight in flights {
                group.addTask {
                    ...
                    var totalApproaches = 0, totalHolds = 0
                    for eligibleFlight in eligibleFlights {
                        ...
                        totalHolds += await (self.flightCountsForIFRCurrency(eligibleFlight) ? (eligibleFlight.flight.holds ?? 0) : 0)
                    }
    ...
    private func flightsWithinLast(days: Int, ofFlight flight: FlightInfo, matchingCategory: Bool = false, matchingClass: Bool = false, matchingTypeIfRequired: Bool = false) async throws -> Array<FlightInfo> {
        ...
        return try flights.filter { f in
            let differentFlight = f != flight,
                ...
                matchesType = try (!matchingTypeIfRequired || !flight.typeRatingRequired() || self.matchesType(current: flight, past: f))
